...
Der zugehörige statistische Test (Breusch-Pagan Test) sollte nicht signifikant sein => Nullhypothese der Homoskedastizität wird angenommen.
```{r}
...
Der zugehörige statistische Test (Durbin-Watson sollte nicht signifikant sein)
...
Ausreißer könnten die Schätzung der Koeffizienten verzerren. Ein Hinweis ob dies der Fall ist, liefert bereits das Streudiagramm. Alternativ können auch Kennzahlen verwendet werden.
Cook's distance ist ein gutes Maß zur Ermittlung zu einflussreicher Ausreißer. Die Werte der einzelnen Fälle sollten nicht größer sein als 4/(N-k-1). Hier im Beispiel N=146 => Werte > 0,0273. 
