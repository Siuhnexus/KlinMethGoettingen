...
Bitte anpassen, damit es für Sie passt. Am einfachsten erst die RMD Datei am Zielort speichern und von dort aus öffnen. Dann unter Session -\> Set Working Directory -\> To Source file location Der Pfad erscheint unten in der Console. Dann über Strg c und Strg v hierher kopieren.
...
ggplot(dat_1, aes(Gruppe, Pre)) + 
  geom_violin() + 
  geom_jitter(width = 0.2, height = 0, col = "grey", size = 0.5) +
...
ggplot(dat_1, aes(Gruppe, Post)) + 
  geom_violin() + 
  geom_jitter(width = 0.1, height = 0, col = "black", size = 0.5) +
  stat_summary(fun = mean, geom = "point", size = 2, col = "red")  +
  stat_summary(geom="errorbar", fun.data=mean_cl_normal, width=.1, col ="red")+
  ylim(0,40) +
  ylab("Score Anxiety") + 
...
ggplot(dat_1, aes(Pre, Change)) + 
  ...
  ylab("Veränderung Post - Prä") + 
  xlab("Prä-Werte")+
  ggtitle("Zusammenhang Prä und Veränderung pro Gruppe")

```

# 1. Analyseoption

Zu Grunde gelegtes statistisches Modell: Score Pre-Treatment und Behandlung sagen Score Post-Treatment vorher
...
# dient der besseren Interpretierbarkeit
...
In den Folien 2_1 ALM finden Sie Erkläerungen zur Interpretation der Daten

## Schritt 2: Testung ob alle Prädiktoren in das Modell eingeschlossen werden sollten
...
# Hier werden die Teilmodelle explizit definiert und mit dem Gesamtmodell M1 verglichen
...
## Schritt 3: Berechnung der Effektstärken = Varianzaufklärung für die einzelnen Prädiktoren

Die Effektstärken geben an, wie der Beitrag zur Varianz des Kriteriums ist. Die Werte beim partiellen eta-Quadrat sind dabei nicht einfach zu verbalisieren (siehe Folien)

```{r}
eta_squared(VA1)
```

## Schritt 4: Überprüfung, ob die Modellannahmen zutreffen
...
scatterplot(M1$residuals ~ M1$fitted.values) # einfache Residuen
scatterplot(rstandard(M1)~fitted(M1)) # standardisierte Residuen
...
Das ALM geht davon aus, dass Messwerte unabhängig voneinander sind. Wenn diese abhängig voneinander sein sollten, dann nimmt das ALM an, dass diese Abhängigkeit durch die Prädiktoren im Modell erklärt wird. Um zu untersuchen, ob dies der Fall ist, wird analysiert, ob die Residuen miteinander korreliert sind. Dies sollte nicht der Fall sein. Wenn diese korreliert sind, dann müssen wir das Modell so erweitern, dass diese Abhängigkeit durch das erweiterte Modell erklärt wird. (Siehe Multi-level) Der zugehörige statistische Test (Durbin-Watson sollte nicht signifikant sein)
...
Ausreißer könnten die Schätzung der Koeffizienten verzerren. Ein Hinweis ob dies der Fall ist, liefert bereits das Streudiagramm. Alternativ können auch Kennzahlen verwendet werden. Cook's distance ist ein gutes Maß zur Ermittlung zu einflussreicher Ausreißer. Die Werte der einzelnen Fälle sollten nicht größer sein als 4/(N-k-1). Hier im Beispiel N=146 =\> Werte \> 0,0273.
...
Es gibt einige Ausreißer, wenn man das oben genannte Kriterium anwendet. Die Summe ist aber bei 1, was gut ist. Da das Scatterplot nichts problematisches zeigt, würde ich keine Werte ausschließen.
...
```{r}
M2 <- lm(Change ~ Gruppe + Pre_c, data = dat_1)
summary(M2)
```
...
```{r}
durbinWatsonTest(M2) # Test für eine Autokorrelation 1.Ordnung
```
