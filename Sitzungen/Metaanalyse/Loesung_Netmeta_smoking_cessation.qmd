...
mit netmeasures(MODELLNAME)
proportion = direct evidence proportion of each network estimate
meanpath = mean path length of each network estimate, should be low, ideally below two
H.tilde = Flow of evidence from one design to another. Shows which designs have strong influence on total network. 

```{r}
netmeasures(m2)
```
## Funnel plot
Auch für die Netzwerkmetaanalyse lassen sich funnelplots erstellen. Bei diesen werden die Daten getrennt für die einzelnen Vergleiche dargestellt. 
linreg und rank stehen für Signifikanztests für die Asymmetrie. 
Bitte in die Hilfe für genauere Informationen schauen.
