...
Der Befehl von oben gibt aus, was das aktuelle Working directory ist. Im Fall dieses Dokuments ist es der Ordner auf meiner Festplatte, in dem diese Datei liegt. Meist sind Daten auch in diesem Ordner und können mit relativen Pfaden einfach angesteuert werden.
...
Sollte es einen Fehler geben, drückt **STRG + SHIFT + H** oder **CMD + SHIFT + H** bei Macs, navigiert zu dem Ordner, indem die Daten liegen, und drückt auf "Öffnen". Probiert es dann nochmal.
...
Wenn man **../** schreibt, bedeutet das "Wechsle in den Überordner des aktuellen Ordners". Wir gelangen hier also aus dem Sitzungen-Ordner in den KlinMethGoettingen-Ordner.
...
**Whitespace-Trennung**: Manchmal werden Dateien weder mit ".", noch mit ";", sondern mit einem Leerzeichen oder einem Tabulator getrennt (keine Sorge wenn ihr nicht wisst, was ein Tabulator sein soll, ist nicht weiter wichtig). Unter diesen Umständen können wir zu data.table wechseln. Diese Funktion macht keine Vorannahmen über die Formatierung, deshalb müssen wir ihr auch das Dezimaltrennzeichen mitteilen und einstellen, ob die Datei Spaltennamen enthält oder nicht.
...
Testweise könnt ihr mal ausprobieren, was passiert, wenn wir die Daten einlesen. Richtig eingelesene Daten haben 500 observations, 4 Variablen und NAs werden beim Anschauen des Datensatzes mit einem kursiv gedruckten *NA* angezeigt.
